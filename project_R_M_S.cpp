...
struct node* order_by_customer(struct node *head,int data,int quantity)
{
    ...
    if(flag==1)
    {
        ...
        struct node *temp = head;
        if(temp==NULL)
            head_Cust = tail_Cust = newnode; //1st order from customer
        else
        {       while(temp->next!=NULL)
                temp=temp->next;
...
struct node* totalsales(int data,int quantity)
{
    ...
    struct node *temp = head_sale;//sale list
    if(temp==NULL)
    ...
    else
    {
        ...
        if(flag==1)
        {
            temp->quantity += newnode-> quantity; //if same order occur in the sale list just add that fooditem with separate node
...
void admin()
{
    ...
    {   adminmenu();
        ...
        switch (opt)
        {
            ...
            case 2:
                if(head_adm==NULL)
                {   cout<<"No Food item is available";
                    break;
                }
